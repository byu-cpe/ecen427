...
The SD card must have a valid system image in order for Linux to run.  Download the class system image from either of these links (they are the same file):
  * [Box](https://byu.box.com/s/by99qytafyapz77r9s3m3bfppd4p31fs)
  * [Google Drive](https://drive.google.com/file/d/13wDopeFtE8B3kbTOYlREtOlTIXvYaa36/view?usp=sharing)

Unzip it after you download it.  The official PYNQ documentation has a guide to [writing the SD card image](https://pynq.readthedocs.io/en/latest/appendix/sdcard.html) that you should follow. Some notes:
...
* You probably won't have space to unzip the .img file onto your home directory, so instead extract it to the `/tmp` folder on the computers.  This is a local folder that is cleared when you log out, so you don't have to worry about filling up the hard drive. Example:
      ...
      dd bs=4M if=/tmp/427.img of=/dev/sda

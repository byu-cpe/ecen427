...
The SD card must have a valid system image in order for Linux to run.  A download link for the class system image is TBD.  Unzip it after you download it.  The official PYNQ documentation has a guide to [writing the SD card image](https://pynq.readthedocs.io/en/latest/appendix/sdcard.html) that you should follow. Some notes:

...
To handle and respond to interrupts generated by the audio hardware, you will do the following:
  * Query the device tree and get the virtual IRQ number.  Note, the device tree lists the physical interrupt line; however, Linux will provide you with a virtual number to use instead  (`platform_get_irq()`, Milestone 1).
...
In this milestone you will create the skeleton of your kernel driver: create your character device, gain access to the device registers, and hook up the hardware interrupt.  You will also create a simple user space program to test your driver.  
...
Your kernel driver needs to:
  ...
  * Also upon device probe, the driver should:
  ...
  * Provide helper functions to read and write registers in the audio device.
  * At the end of your probe function, enable the interrupt output of the audio core.  Since there is no data in the FIFOs, this should immediately trigger your ISR.  In your ISR print a message to the kernel log and then disable the interrupt output of the audio core (or you will be stuck in an endless loop).  
  * Make sure that:
...
## Code Submission 
Follow the [Submission Instructions]({% link _pages/submission.md %}).  Reminder: your code must compile with zero warnings.  The `valgrind` requirement does not apply to kernel modules.
...
Some resources to help you with the kernel function calls:
  ...
  * `platform_get_irq`
    * Use this, not `platform_get_resource` with `IORESOURCE_IRQ`, to retrieve the IRQ number.  (*Note:* It will be a virtual IRQ number, and will not be the same as the physical IRQ number listed in the device tree).
